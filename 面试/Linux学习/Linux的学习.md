...
> 数值运算 (( )) 这种写法，对括号内的表达式求值，这样声明了一个整形数据
>
> 1.自定义变量赋值时，“=”左右两边不能有空格
>
> 2.BASH中的语句结尾不需要分号
>
> 3.若变量内容又用户决定，可以使用“read”命令从终端键盘获取用户的输入值
...
> 注意：
>
> 1.第一个方括号左右两边分别有一个空格，第二个方括号的左右两边也有一个空格，符号“==”与两边的组件之间也有一个空格
>
> 2.中括号[ ]内的变量最好使用双引号来设置
...
> 1.case语句最后必须以esac结尾
>
> 2.每个变量内容的程序段最后都需要2个分号结尾
>
> 3.最后的“ * ”作用表示默认的意思

##### 3.4 循环(wiile...do...done)

> 循环while...do...done的格式
>
> 当判断条件成立,就进行循环,直到条件不成立停止
...
> 2.continue命令
>
> continue命令与break命令累死，它不会跳出所有循环，仅仅跳出当前循环，当前循环结束后会继续
...
> while ：	这个冒号代表如果条件结果为真，那么就会一直循环下去，直至条件结果为假为止。
>
> while和冒号之间有一个空格
...
> bash(Bourne Again shell)也跟其他变成语言一样支持函数，一般在编写大型脚本中需要用到，它可以将shell脚本程序划分成一个个功能相对独立的代码块，使代码模块化更好，结构更清晰，并介绍程序的代码量

> 但是bash作为一种解释性语言，bash在变成能力方面提供的支持并不像其他变异性语言那样完善，执行效率也更低下
...
> ```
>
> 可以带function函数名()定义，也可以直接函数名()定义，不带任何参数
...
> }
> echo $dd	
> #这里打印不生效，因为dd是局部变量，需要在函数内使用
> echo $aa
> #这里打印生效，因为aa是全局变量，所有地方都可以使用
> test	# 调用函数test
> ```
...
fun 1 2 3 4 5 6 7 8 9 67 98
```

> $1表示拿的是fun函数中的第一个参数，以此类推
>
> $#获取参数总数
>
> $*获取所有参数内容，以字符串格式打印
